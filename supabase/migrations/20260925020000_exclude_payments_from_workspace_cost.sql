-- Keep settlement payments out of workspace expense totals while applying them to balances.
...
begin
  ...
  loop
    if e_record.category is null
      or (e_record.category != 'Payment' and e_record.category != 'Settlement') then
  ...
  while c_idx < c_len and d_idx < d_len loop
    ...
    if abs(creditor_bal) < 0.01 then c_idx := c_idx + 1; end if;
    if abs(debtor_bal) < 0.01 then d_idx := d_idx + 1; end if;
  end loop;
