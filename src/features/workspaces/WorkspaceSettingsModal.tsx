...
export const WorkspaceSettingsModal: React.FC<WorkspaceSettingsModalProps> = ({
  ...
  const handleRemoveMember = async (userId: string) => {
    if (userId === workspace.owner_id) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
      <div className="animate-scale-up relative w-full max-w-lg rounded-3xl border border-border-subtle bg-surface p-6 shadow-2xl text-text-primary md:p-8">
        <div className="mb-6 flex items-center justify-between border-b border-border-subtle pb-4">
          ...
          <button
            onClick={onClose}
            className="cursor-pointer rounded-xl p-1.5 text-text-muted transition-colors hover:bg-surface-subtle hover:text-text-primary"
          >
        ...
        {errorMessage && (
          <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-700 [data-theme='dark']_&:border-rose-900/50 [data-theme='dark']_&:bg-rose-950/40 [data-theme='dark']_&:text-rose-300">
            {errorMessage}
          </div>
        )}

        {successMessage && (
          <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-semibold text-emerald-700 [data-theme='dark']_&:border-emerald-900/50 [data-theme='dark']_&:bg-emerald-950/40 [data-theme='dark']_&:text-emerald-300">
            {successMessage}
          </div>
        )}

        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:divide-x md:divide-border-subtle">
          {/* Form settings */}
          <form onSubmit={handleSave} className="space-y-4 md:pr-6">
            ...
            <div>
              ...
              <input
                ...
                className="focus:ring-primary-green/15 focus:border-primary-green w-full rounded-xl border border-border-subtle bg-surface-subtle px-3 py-2 text-xs font-semibold text-text-primary outline-hidden transition-all focus:bg-surface focus:ring-2"
                required
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="focus:ring-primary-green/15 focus:border-primary-green w-full rounded-xl border border-border-subtle bg-surface-subtle px-2.5 py-2 text-xs font-semibold text-text-primary outline-hidden transition-all focus:bg-surface focus:ring-2"
                required
              >
            ...
            <div>
              ...
              <div className="space-y-1.5">
                <input
                  ...
                  className={`focus:ring-primary-green/15 focus:border-primary-green w-full rounded-xl border px-3 py-2 text-xs font-semibold outline-hidden transition-all focus:bg-surface focus:ring-2 ${
                    isLimitInvalid
            ...
            {currentUserId === workspace.owner_id && (
              <div className="mt-6 border-t border-rose-500/20 pt-4">
                <h5 className="text-xs font-bold text-rose-600 [data-theme='dark']_&:text-rose-400">Danger Zone</h5>
          ...
          <div className="space-y-4 md:pl-6">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-md bg-surface-subtle px-2 py-0.5 text-[9px] font-bold text-text-secondary">
            ...
            <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
              {members.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className="flex items-center justify-between rounded-xl border border-border-subtle bg-surface-subtle/50 p-2 transition-all hover:bg-surface-subtle"
                  >
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-xs leading-tight font-semibold text-text-primary">
                          {member.display_name} {isCurrentUser ? '(You)' : ''}
                        </p>
                        {isMemberOwner && (
                          <span className="mt-0.5 inline-block rounded-md border border-emerald-500/20 bg-emerald-500/10 px-1 py-0.5 text-[8px] leading-none font-bold text-emerald-600 [data-theme='dark']_&:text-emerald-400">
                            Owner
                          </span>
                        )}
                      </div>
                    ...
                    {!isMemberOwner && (
                      <button
                        ...
                        className="cursor-pointer rounded-lg p-1.5 text-text-muted transition-colors hover:bg-rose-500/15 hover:text-rose-600 disabled:pointer-events-none"
