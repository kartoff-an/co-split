...
export const WorkspaceSettingsModal: React.FC<WorkspaceSettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
      <div className="animate-scale-up border-border-subtle bg-surface text-text-primary relative w-full max-w-lg rounded-3xl border p-6 shadow-2xl md:p-8">
        <div className="border-border-subtle mb-6 flex items-center justify-between border-b pb-4">
          ...
          <button
            onClick={onClose}
            className="text-text-muted hover:bg-surface-subtle hover:text-text-primary cursor-pointer rounded-xl p-1.5 transition-colors"
          >
        ...
        {errorMessage && (
          <div className="[data-theme='dark']_&:border-rose-900/50 [data-theme='dark']_&:bg-rose-950/40 [data-theme='dark']_&:text-rose-300 mb-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-700">
            {errorMessage}
          </div>
        )}

        {successMessage && (
          <div className="[data-theme='dark']_&:border-emerald-900/50 [data-theme='dark']_&:bg-emerald-950/40 [data-theme='dark']_&:text-emerald-300 mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-semibold text-emerald-700">
            {successMessage}
          </div>
        )}

        <div className="md:divide-border-subtle grid grid-cols-1 gap-6 md:grid-cols-2 md:divide-x">
          {/* Form settings */}
          <form onSubmit={handleSave} className="space-y-4 md:pr-6">
            ...
            {currentUserId === workspace.owner_id && (
              <div className="mt-6 border-t border-rose-500/20 pt-4">
                <h5 className="[data-theme='dark']_&:text-rose-400 text-xs font-bold text-rose-600">
                  Danger Zone
                </h5>
            ...
            <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
              {members.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className="border-border-subtle bg-surface-subtle/50 hover:bg-surface-subtle flex items-center justify-between rounded-xl border p-2 transition-all"
                  >
                    <div className="flex min-w-0 items-center gap-2">
                      {member.avatar_url ? (
                        <img
                          ...
                          className="border-border-subtle h-6 w-6 rounded-full border"
                        />
                      ) : (
                      ...
                      <div className="min-w-0">
                        ...
                        {isMemberOwner && (
                          <span className="[data-theme='dark']_&:text-emerald-400 mt-0.5 inline-block rounded-md border border-emerald-500/20 bg-emerald-500/10 px-1 py-0.5 text-[8px] leading-none font-bold text-emerald-600">
                            Owner
                          </span>
                        )}
                      </div>
                    ...
                    {!isMemberOwner && (
                      <button
                        ...
                        className="text-text-muted cursor-pointer rounded-lg p-1.5 transition-colors hover:bg-rose-500/15 hover:text-rose-600 disabled:pointer-events-none"
